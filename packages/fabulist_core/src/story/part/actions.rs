use crate::{
    error::EngineResult,
    story::{context::Mappable, reference::ListKey},
};

pub type QueryNextClosure = Box<dyn Fn(&dyn Mappable) -> EngineResult<ListKey<String>>>;
pub trait QueryNext {
    fn query_next(&self) -> Option<&QueryNextClosure>;
    fn set_query_next(&mut self, closure: QueryNextClosure);
}

pub type ChangeContextClosure = Box<dyn Fn(&mut dyn Mappable) -> EngineResult<()>>;
